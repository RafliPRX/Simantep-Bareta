<?php
    ...
    $pdf->Image('tanda_tangan.png', 115, 173, 80);
    $pdf->Cell(25,7,'',0,1);
    $pdf->Cell(25,7,'',0,0);
    $pdf->Cell(25,7,'',0,1);
    $pdf->Cell(25,7,'',0,0);
    ...
    // $pdf->Cell(15,5,'',0,1,'C');
    $print = "SELECT a.id_surat, a.nama, a.nrk, b.nama_jabatan, alamat, a.cuti1, a.cuti_date, a.cuti_date_fin, a.date_now FROM surat a, jabatan b WHERE a.id_jabatan=b.id_jabatan AND a.id_surat='$id'";
    $res = mysqli_query($konek, $print);
    $row = mysqli_fetch_array($res);
    $tanggal_awal = $row['cuti_date'];
    $awal = date('d', strtotime($tanggal_awal));
    $tanggal_akhir = $row['cuti_date_fin'];
    ...
    $tanggal_akhir1 = $bulan[date('n', strtotime($tanggal_akhir))];
    $akhir = date('d', strtotime($tanggal_akhir)) . ' ' . $tanggal_akhir1;
    $bulan_akhir = $bulan[date('n', strtotime($tanggal_sekarang))];
    $sekarang = date('d', strtotime($tanggal_sekarang)) . ' ' . $bulan_akhir . ' ' . date('Y', strtotime($tanggal_sekarang));
    // var_dump($sekarang);
    // die;
    $pdf->Cell(25,7,'',0,1);
    $pdf->Cell(25,7,'',0,0);
    ...
    $pdf->Cell(10,7,'',0,0);
    $pdf->Cell(180,7,'Selama '.$row['cuti1'].' hari kerja, terhitung mulai tanggal '.$awal.' sampai '.$akhir.' dengan ketentuan sebagai berikut :',0,1);
    $pdf->Cell(10,7,'',0,0);
    $pdf->Cell(10,7,'',0,1);
    $pdf->Cell(10,7,'',0,0);
    $pdf->Cell(6,7,'a.',0,0);
    $pdf->Cell(180,7,'Sebelum menjalankan cuti wajib menyerahkan pekerjaanya kepada atasan langsungnya;',0,1);
    $pdf->Cell(10,7,'',0,0);
    $pdf->Cell(6,7,'b.',0,0);
    $pdf->Cell(180,7,'Setelah selesai menjalankan cuti wajib melaporkan diri kepada atasan langsungnya dan',0,1);
    $pdf->Cell(10,7,'',0,0);
    $pdf->Cell(6,7,'',0,0);
    $pdf->Cell(180,7,'bekerja kembali sebagaimana biasa;',0,1);
    $pdf->Cell(10,7,'',0,0);
    $pdf->Cell(6,7,'c.',0,0);
    $pdf->Cell(180,7,'Selama menjalankan cuti wajib menjaga protokol kesehatan.',0,1);
    $pdf->Cell(10,7,'',0,1);
    $pdf->Cell(10,7,'2.',0,0);
